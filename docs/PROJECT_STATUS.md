...
- Notification center redesign with `Unread` first by default, `All` as the full 30-day history, persisted read receipts backed by stable feed IDs, an unread-count badge on the Alerts nav item capped at `9+`, SOS detail popups on tap, SOS feed suppression for the caller, and pull-to-refresh timestamp refresh.
