...
- Separate `admin-web/` React + Vite back-office MVP for custom-claim protected operations: admin login, dashboard counts, hotline management, evacuation-center management, support/problem-report review, admin access management, audit logs, and role management.
- Cloud Functions now expose a custom-claim protected admin back-office surface for hotlines, evacuation centers, support reports, admin access, role management, and audit logs without relaxing mobile Firestore rules.
- The admin-web notification bell now loads a newest-first, role-filtered operational queue from Cloud Functions, opens the affected report/center/hotline modal directly, and supports per-admin persistent individual dismissal and bulk clearing without changing operational records.
- Operator script support for assigning `sosyncRole` custom claims (`superadmin`, `admin`, or `operator`) to Firebase Auth users before admin portal use.
...
- Home screen redesign with a full-screen Android Google Navigation SDK map, restored SOSync light/dark map styling, local circular avatar/member marker icons, local evacuation-center marker icons, a draggable list-driven bottom sheet tuned for longer circle and contact lists, a compact current-weather preview beside the Home `Share Live` / `Pause Live` control, the shared bottom tab bar, coordinated light/dark Home-scene variants, mounted tab persistence so Home returns without a broad map reset, a synchronized fast fade for the top pill and right-side quick-member stack as the sheet reaches its top snap point, eager tab mounting plus stable marker/alert prop signatures to keep the map surface warm, a narrower top pill that fades out quickly only when the sheet reaches the top, centered chip-specific shadows for trusted-circle pills, a clean bottom-sheet ending without the extra white footer tail, tap-to-call confirmation from Home contact names, and a shadow-first Home surface treatment across the top pill, floating CTAs, and bottom sheet.
- The Home Navigation SDK surface now applies the previous custom Google map style JSON and uses cached Android-generated PNG icons for circular profile markers, initials fallbacks, and static safety-hub markers instead of default Google red pins. Evacuation-center taps now only reveal a React-rendered nametag; the in-app route preview opens from the nametag direction arrow rather than from the marker or label text.
...
- The flood outlook sheet now uses a richer Google Flood Forecasting payload with a clear severity ladder (`SAFE` through `EXTREME DANGER` plus `LIMITED DATA`), a simplified alert-first UI, flatter inline metadata for trend/update time, one primary monitoring point, up to three nearby alternatives, a stable plain-language risk guide without raw threshold numbers, and a popup modal for nearby-point details. The previous native mini-map preview is disabled while Home uses Google Navigation SDK because the Navigation SDK and `react-native-maps`/Maps SDK cannot safely coexist in the same Android app.
...
- Home weather and flood outlooks now respect the in-app `Share Live` privacy toggle: when location sharing is paused, cached coordinates are not used for outlook calls or stale weather/flood display, and the Home preview plus both sheets show a location-off state instead.
- Home now distinguishes member presence from privacy: members who keep sharing live but stop updating for more than 10 minutes remain on their last known map position with `Offline · last seen Xm ago`, while paused sharing remains hidden.
- Nearest Safety Hub on Home now uses location-scoped evacuation centers and Android in-app Google Navigation SDK guidance: the app loads nearby function-filtered centers for the user's current area, Home displays only hubs within 2 km of the user while keeping an empty-state card when none qualify, the nearest-hub quick action focuses the closest visible hub, the Home footer now exposes only a single `Navigate` CTA, and SOSync opens a Google-Maps-like route preview bottom sheet before starting built-in turn-by-turn guidance inside the app.
- Admin-managed evacuation centers now store `countryCode: PH` separately from an official 18-option PSA `regionCode`/`region`, derive Island group immediately for both place search and manual selection, and use City/Municipality plus Region without a separate Province field. Center contacts accept readable mobile or landline formatting with 7 to 11 total digits. Nearby-center endpoints temporarily merge the new country query with legacy `region: PH` records, deduplicate results, and exclude disabled centers, so these admin changes do not require a mobile rebuild.
- Starting in-app evacuation guidance is now server-authorized per signed-in user with a fixed-window limit of 5 guidance starts per 5 minutes, shared across nearby centers and travel-mode restarts, so the Home navigation surface can block rapid repeated requests with a clear wait-time message instead of hammering backend/navigation setup.
- Live and fallback evacuation-center seeds now use named Manila and Talisay/Cebu sites instead of generic placeholders, including Don Bosco School, Dapitan Sports Complex, Talisay Sports Academy Center, and Tabunok Barangay Hall.
- Full-page SOS redesign with responsive concentric rings, orbiting member avatars, a tap-or-hold trigger, a left-to-right slide-to-cancel control during countdown, trusted-circle alert fanout without any embedded hotline UI, and a send gate that requires both live location sharing and Auto-share location on SOS before arming or sending.
- User-facing offline/backend messaging is now hardened across SOS, OTP verification/reset requests, circle create/join, flood refresh, and profile save so live-mode failures show plain-language reconnect/retry copy instead of Firebase, Firestore, Cloud Firestore, project setup, or raw backend error terms.
- Notification center redesign with `Unread` first by default, `All` as the full 30-day history for the user's active membership, persisted read receipts backed by stable feed IDs, an unread-count badge on the Alerts nav item capped at `9+`, SOS detail popups on tap, SOS feed suppression for the caller, join-time filtering so new circle members do not see pre-join alerts/SOS history, and pull-to-refresh timestamp refresh.
- Weather alert details now use an app-matched detail page with back navigation, an at-a-glance summary, recommended steps, a readable forecast window, reverse-geocoded area labels, source details, and model metadata instead of a single generic text card.
...
- Cloud Functions for nearby evacuation-center filtering, dormant route proxying, alert sync, permanent circle creation/join, owner/admin/member circle management, and push fanout.
- Scheduled weather alert sync now de-duplicates ongoing conditions by group, alert type, severity, and forecast day so repeated syncs update the active alert instead of creating repeated push-triggering documents.
- Cloud Functions now also accept authenticated support requests and problem reports into a backend review queue, with report media stored under owner-scoped Firebase Storage paths.
...
- Firestore rules explicitly block direct client access to `support_reports`; support and problem reports must go through authenticated Cloud Functions.
- Storage rules now cover owner-scoped support report media uploads under `supportReports/{userId}/{reportId}/...`.
- Admin support-report details now request fresh 15-minute signed media URLs on demand, expose missing/unavailable preview states, and provide an in-app image/video viewer without making Storage objects public.
...
- Unsupported `message` notification payloads are ignored for release because no complete in-app messaging feature is currently shipped.
...
- Profile-write retry tests cover the fresh signup `permission-denied` race, including UID mismatch and missing-auth guardrails.
